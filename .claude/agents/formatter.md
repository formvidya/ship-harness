...
```
python tools/harness/context-harness/ctx/ctx.py decide --pr <n> --agent formatter \
...
<!-- Rendered from tools/harness/methodology-harness/templates/agents/_scaffold.md.tmpl by render_agents.py.
     Edit the template (shared) or .context/agents/formatter.md (this agent's body), then
