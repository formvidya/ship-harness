...
```
python tools/harness/context-harness/ctx/ctx.py decide --pr <n> --agent security \
...
<!-- Rendered from tools/harness/methodology-harness/templates/agents/_scaffold.md.tmpl by render_agents.py.
     Edit the template (shared) or .context/agents/security.md (this agent's body), then
