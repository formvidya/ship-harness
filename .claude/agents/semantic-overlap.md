...
```
python tools/harness/context-harness/ctx/ctx.py decide --pr <n> --agent semantic-overlap \
...
<!-- Rendered from tools/harness/methodology-harness/templates/agents/_scaffold.md.tmpl by render_agents.py.
     Edit the template (shared) or .context/agents/semantic-overlap.md (this agent's body), then
